...
class AdaptiveThresholds:
    ...
    def update(self, pair: str, confidence: float, adx: float, reason: str = "") -> ThresholdUpdate:
        ...
        return update
    def compute_adjustment(self, pair: str, feedback_scores: List[int]) -> Optional[Dict]:
        """
        Calcula el ajuste candidato SIN persistirlo — permite que capas superiores
        (ej. constitucion/Fase 8) validen el resultado ANTES de escribirlo en DB.
        Retorna dict con old/new confidence/adx + reason, o None si no aplica.
        """
        ...
        return {
            "pair": pair,
            "old_confidence": current["confidence"], "new_confidence": round(max(_MIN_CONFIDENCE, min(_MAX_CONFIDENCE, conf)), 4),
            "old_adx": current["adx"], "new_adx": round(max(_MIN_ADX, min(_MAX_ADX, adx)), 2),
            "reason": "; ".join(reason_parts),
        }

    def adapt_from_feedback(self, pair: str, feedback_scores: List[int]) -> Optional[ThresholdUpdate]:
        """Calcula Y persiste directamente (comportamiento historico, sin gate constitucional)."""
        calc = self.compute_adjustment(pair, feedback_scores)
        if calc is None: return None
        return self.update(pair, calc["new_confidence"], calc["new_adx"], calc["reason"])
