...
class ConstitutionValidator:
    """
    NOTA DE INTEGRACION (Fase 8, auditoria real): las propuestas reales que genera
    evolution/evolution_proposal.py llevan un payload generico
    {"action","component","evidence","suggested_cmd"} — NUNCA incluyen claves
    numericas como "new_confidence"/"new_adx"/"wfv_passed"/"avg_precision" al
    momento de CREAR la propuesta, porque esos valores concretos (ej. el nuevo
    umbral tras adaptar por feedback) solo se conocen al momento de APLICAR.
    Por eso este validador, llamado en submit()/ApprovalFlow al momento de
    aprobar, casi nunca encuentra violaciones de min_confidence_threshold /
    min_adx_threshold — son en la practica no-ops en esta etapa.
    El enforcement REAL y efectivo de esos dos limites ocurre en
    evolution_proposal.py -> EvolutionProposal.apply() para proposal_type
    "threshold_adjust": ahi se calcula el ajuste candidato (compute_adjustment,
    sin persistir) y se valida contra estas mismas reglas ANTES de escribirlo
    en DB, bloqueando (status='rejected') si viola el minimo constitucional.
    """
