...
BUILTIN_RULES = [
    ...
    # BUGFIX: el borrador traía 0.55 como piso, pero el predictor real (forex/prediction/predictor.py,
    # MIN_CONFIDENCE) usa 0.65 en producción. Se alinea la constitución al valor REAL para que
    # esta regla constitucional no permita debilitar el sistema por debajo de su propio estándar.
    Rule("min_confidence_threshold","Confianza mínima","El sistema nunca puede emitir señal con confianza < 65%","hard_limit",{"field":"confidence","min_value":0.65}),
    # BUGFIX: el borrador traía 18.0, pero el predictor real usa MIN_ADX=22.0 en producción.
    Rule("min_adx_threshold","ADX mínimo","No emitir señales en mercados ranging (ADX < 22)","hard_limit",{"field":"adx","min_value":22.0}),
