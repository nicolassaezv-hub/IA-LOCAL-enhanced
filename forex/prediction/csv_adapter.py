...
def merge_mtf_context(df_h1: pd.DataFrame,
    ...
    def _load_tf(path, prefix, cols):
        ...
        if "RSI_14" not in raw.columns and "close" in raw.columns:
            ...
            # ATR 14
            if all(col in raw.columns for col in ["high", "low"]):
                tr = pd.concat([raw["high"] - raw["low"],
                                (raw["high"] - c.shift()).abs(),
                                (raw["low"] - c.shift()).abs()], axis=1).max(axis=1)
                raw["ATR_14"] = tr.rolling(14).mean()
            # EMAs
            raw["EMA20"] = c.ewm(span=20, adjust=False).mean()
            raw["EMA50"] = c.ewm(span=50, adjust=False).mean()
            raw["EMA200"] = c.ewm(span=200, adjust=False).mean()
            # MACD
            ema12 = c.ewm(span=12, adjust=False).mean()
            ema26 = c.ewm(span=26, adjust=False).mean()
            raw["MACD"] = ema12 - ema26
            # Returns
            raw["returns"] = c.pct_change()
        ...
        rename = {
            ...
            "ema_200":   f"{prefix}_ema200",
            "MACD":      f"{prefix}_macd",
            "macd":      f"{prefix}_macd",
            "returns":   f"{prefix}_return5",
